...
export const GUIDE_HEADING =
  "m-0 mb-4 mt-12 text-[24px] font-semibold tracking-[-.025em] text-(--ink)";
export const GUIDE_PARAGRAPH =
  "m-0 mb-5 text-[16px] leading-[1.75] text-(--muted)";
export const GUIDE_CODE =
  "mb-5 overflow-x-auto rounded-lg border border-(--border) bg-(--code-bg) p-4 font-mono text-[13px] leading-[1.7] text-(--ink)";
  ...
  return (
    <div className="min-h-screen bg-(--canvas) text-(--ink)">
      ...
      <main className="mx-auto max-w-215 px-7 pb-24 pt-14">
        ...
        <p className="m-0 mb-12 max-w-180 text-[19px] leading-[1.65] text-(--muted)">
          {intro}
        </p>
...
export function GuideList({ children }: { children: ReactNode }) {
  return (
    <ul className="m-0 mb-5 list-disc space-y-2 pl-6 text-[16px] leading-[1.7] text-(--muted)">
      {children}
    </ul>
  );
}
...
export function GuideCallout({ children }: { children: ReactNode }) {
  return (
    <aside className="my-8 rounded-lg border border-(--accent-line) bg-(--accent-soft) px-5 py-4 text-[15px] leading-[1.65] text-(--ink)">
      {children}
    </aside>
  );
}
...
export function RelatedGuides({
  ...
  return (
    <section className="mt-16 border-t border-(--border) pt-9">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {links.map((link) => (
          <a
            key={link.href}
            href={link.href}
            className="group rounded-xl border border-(--border) bg-(--surface) px-5 py-4 transition-colors hover:border-(--accent-line)"
          >
            <span className="block font-semibold text-(--ink) group-hover:text-(--accent)">
              {link.label} →
            </span>
            <span className="mt-1 block text-sm leading-[1.55] text-(--muted)">
              {link.description}
            </span>
          </a>
        ))}
