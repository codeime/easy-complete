...
#[cfg(target_os = "linux")]
use fastab_integrations::desktop_entry::{AutostartIntegration, DesktopEntryIntegration};
use fastab_integrations::shell::ShellExt;
use fastab_integrations::ssh::SshIntegration;
use fastab_os_shim::{ContextArcProvider, ContextProvider, EnvProvider};
...
use fastab_proto::fig::{InstallAction, InstallComponent, InstallRequest, InstallResponse, Result as ProtoResult};
...
#[allow(dead_code)]
async fn integration_status(integration: impl fastab_integrations::Integration) -> ServerOriginatedSubMessage {
    ...
    Ctx: SettingsProvider + StateProvider + ContextProvider + ContextArcProvider + Send + Sync,
{
    let response = match (request.component(), request.action()) {
        ...
        (InstallComponent::InputMethod, InstallAction::Install) => {
            cfg_if::cfg_if! {
                if #[cfg(target_os = "macos")] {
                    use fastab_integrations::input_method::{
                        InputMethod,
                    };
                    use fastab_integrations::Integration;
...
{
    use fastab_integrations::gnome_extension::GnomeExtensionIntegration;
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    use fastab_integrations::desktop_entry::global_entry_path;
    use fastab_os_shim::{Context, ContextProvider};
    use fastab_proto::fig::server_originated_message::Submessage;
    ...
    impl StateProvider for TestContext {
        fn state(&self) -> &fastab_settings::State {
            &self.state
        }
    }
