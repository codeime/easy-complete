...
fn command_is_disabled(command: &str) -> bool {
    fastab_settings::settings::get::<Vec<String>>("autocomplete.disableForCommands")
        .ok()
        .flatten()
...
fn first_token_result(
    ...
    add_exact_auto_execute(
        ...
        fastab_settings::settings::get_bool_or("autocomplete.hideAutoExecuteSuggestion", false),
        fastab_settings::settings::get_bool_or("autocomplete.onlyShowOnTab", false),
        fastab_settings::settings::get_bool_or("autocomplete.immediatelyRunDangerousCommands", false),
    );
    CompleteResult {
        suggestions,
        fuzzy: request.fuzzy,
...
pub fn complete(registry: &mut Registry, request: &CompleteRequest) -> CompleteResult {
    ...
    let prefer_verbose = fastab_settings::settings::get_bool_or("autocomplete.preferVerboseSuggestions", false);
    ...
    add_current_token_auto_execute(
        ...
        suggest_current_token_for(
            ...
            fastab_settings::settings::get_bool_or("autocomplete.alwaysSuggestCurrentToken", false),
        ),
        fastab_settings::settings::get_bool_or("autocomplete.hideAutoExecuteSuggestion", false),
        fastab_settings::settings::get_bool_or("autocomplete.onlyShowOnTab", false),
        fastab_settings::settings::get_bool_or("autocomplete.immediatelyRunDangerousCommands", false),
    ...
    add_space_auto_execute(
        &mut suggestions,
        &normalized_search_term,
        fastab_settings::settings::get_bool_or("autocomplete.immediatelyExecuteAfterSpace", false),
        fastab_settings::settings::get_bool_or("autocomplete.hideAutoExecuteSuggestion", false),
        fastab_settings::settings::get_bool_or("autocomplete.onlyShowOnTab", false),
    );
...
fn root_spec_for_command(registry: &mut Registry, request: &CompleteRequest, command: &str) -> Option<Arc<Spec>> {
    let shortcuts_token = fastab_settings::settings::get_string_or("autocomplete.personalShortcutsToken", "+".into());
    if command == "?" {
