...
use fastab_util::directories::{fastab_data_dir_ctx, local_webview_data_dir};
use fastab_util::manifest::manifest;
use fastab_util::{CLI_BINARY_NAME, PRODUCT_NAME};
...
pub(crate) async fn update(
    ...
    match &manifest().variant {
        fastab_util::manifest::Variant::Full => update_full(update_package, tx, interactive, relaunch_dashboard).await,
        fastab_util::manifest::Variant::Minimal => {
            update_minimal(update_package, tx, interactive, relaunch_dashboard).await
        },
        fastab_util::manifest::Variant::Other(other) => Err(Error::UnsupportedVariant(other.clone())),
    }
}
...
pub(crate) async fn update_minimal(
    ...
    let local_bin = fastab_util::directories::home_local_bin()?.canonicalize()?;
...
async fn update_full_ctx(
    ...
    let lock_file_path = fastab_util::directories::update_lock_path(ctx)?;
...
pub(crate) async fn uninstall_desktop(ctx: &Context) -> Result<(), Error> {
    let fs = ctx.fs();
    let data_dir_path = fastab_data_dir_ctx(fs)?;
...
mod tests {
    ...
    use fastab_test_utils::TestServer;
    use fastab_test_utils::http::Method;
    use fastab_util::CLI_BINARY_NAME;
    ...
    async fn test_uninstall_desktop_removes_data_dir() {
        let ctx = Context::builder()
            ...
            .build_fake();
        let fs = ctx.fs();
        let data_dir_path = fastab_data_dir_ctx(fs).unwrap();
    ...
    async fn test_appimage_updates_successfully() {
        ...
        assert!(
            !ctx.fs()
                .exists(fastab_util::directories::update_lock_path(&ctx).unwrap()),
