...
use fastab_util::manifest::{Channel, manifest};
#[cfg(target_os = "freebsd")]
use freebsd as os;
...
pub enum Error {
    ...
    UnsupportedPlatform,
    #[error(transparent)]
    Util(#[from] fastab_util::Error),
    #[error(transparent)]
    Integration(#[from] fastab_integrations::Error),
    #[error(transparent)]
    Settings(#[from] fastab_settings::Error),
    #[error(transparent)]
...
impl From<fastab_util::directories::DirectoryError> for Error {
    fn from(err: fastab_util::directories::DirectoryError) -> Self {
        fastab_util::Error::Directory(err).into()
    }
}

// The current selected channel
pub fn get_channel() -> Result<Channel, Error> {
    Ok(match fastab_settings::state::get_string("updates.channel")? {
        Some(channel) => Channel::from_str(&channel)?,
        None => {
            let manifest_channel = manifest().default_channel;
            if fastab_settings::settings::get_bool_or("app.beta", false) {
...
pub fn get_max_channel() -> Channel {
    let state_channel = fastab_settings::state::get_string("updates.channel")
        .ok()
        .flatten()
        .and_then(|s| Channel::from_str(&s).ok())
        .unwrap_or(Channel::Stable);
    let manifest_channel = manifest().default_channel;
    let settings_channel = if fastab_settings::settings::get_bool_or("app.beta", false) {
        Channel::Beta
    } else {
        Channel::Stable
    };
