...
use fastab_util::consts::{APP_BUNDLE_ID, CLI_BINARY_NAME, system_paths};
use fastab_util::macos::BUNDLE_CONTENTS_MACOS_PATH;
use fastab_util::{APP_BUNDLE_NAME, directories};
use regex::Regex;
...
pub(crate) async fn update(
    ...
    if !same_bundle_name {
        cmd.arg("--delete-bundle").arg(fastab_util::app_bundle_path());
    }
...
pub(crate) async fn uninstall_desktop(ctx: &fastab_os_shim::Context) -> Result<(), Error> {
    // TODO:
    ...
    // Delete data dir
    if let Ok(fastab_data_dir) = directories::fastab_data_dir() {
        let state = fastab_settings::state::get_string("anonymousId").unwrap_or_default();

        for file in std::fs::read_dir(fastab_data_dir).ok().into_iter().flatten().flatten() {
    ...
    let app_path = fastab_util::app_bundle_path();
    if app_path.exists() {
    ...
    if let Ok(old_fastab_data_dir) = directories::old_fastab_data_dir() {
        if old_fastab_data_dir.exists() {
            if let Ok(metadata) = fs::symlink_metadata(&old_fastab_data_dir).await {
                if metadata.is_symlink() {
                    fs::remove_file(&old_fastab_data_dir)
                        .await
                        .map_err(|err| error!("Failed to remove the old fig data dir {old_fastab_data_dir:?}: {err}"))
                        .ok();
                }
            }
        }
