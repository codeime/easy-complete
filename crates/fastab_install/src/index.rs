...
use fastab_util::PRODUCT_NAME;
use fastab_util::manifest::{Channel, FileType, Os, TargetTriple, Variant, bundle_metadata};
use fastab_util::system_info::get_system_id;
use semver::Version;
...
static RELEASE_URL: LazyLock<Url> = LazyLock::new(|| {
    match fastab_os_shim::Env::new().q_desktop_release_url() {
        Ok(s) => Url::parse(&s),
        Err(_) => match fastab_settings::settings::get_string("install.releaseUrl") {
...
impl Serialize for ProductName {
    ...
    {
        match self {
            ProductName::EasyComplete => serializer.serialize_str(PRODUCT_NAME),
...
pub async fn get_file_type(ctx: &Context, variant: &Variant) -> Result<FileType, Error> {
    match ctx.platform().os() {
        fastab_os_shim::Os::Mac => Ok(FileType::Dmg),
        fastab_os_shim::Os::Linux => match variant {
...
mod tests {
    ...
    use fastab_util::{OLD_CLI_BINARY_NAMES, OLD_PRODUCT_NAME, PRODUCT_NAME};
    ...
    fn test_product_name_ser_deser() {
        test_ser_deser!(ProductName, ProductName::EasyComplete, PRODUCT_NAME);
