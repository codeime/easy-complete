...
fn apply_overlay_hints(conn: &RustConnection, screen_num: usize, window: xproto::Window) {
    let Some(net_wm_state) = intern(conn, b"_NET_WM_STATE") else {
        return;
    };
    let Some(above) = intern(conn, b"_NET_WM_STATE_ABOVE") else {
        return;
    };
    let Some(skip_taskbar) = intern(conn, b"_NET_WM_STATE_SKIP_TASKBAR") else {
        return;
    };
    let Some(skip_pager) = intern(conn, b"_NET_WM_STATE_SKIP_PAGER") else {
        return;
    };
    ...
    let event = xproto::ClientMessageEvent::new(32, window, net_wm_state, [1, above, skip_taskbar, 0, 0]);
