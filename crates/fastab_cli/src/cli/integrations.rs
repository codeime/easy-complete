...
use eyre::Result;
use fastab_integrations::Integration as _;
...
async fn install(integration: Integration, silent: bool) -> Result<()> {
        ...
        Integration::InputMethod => {
            cfg_if::cfg_if! {
                if #[cfg(target_os = "macos")] {
                    ...
                    fastab_integrations::input_method::InputMethod::default().install().await?;
                    installed = true;
    ...
    if installed && result.is_ok() {
        fastab_telemetry::track_blocking(
...
async fn uninstall(integration: Integration, silent: bool) -> Result<()> {
    ...
    let result = match integration {
        ...
        Integration::InputMethod => {
            cfg_if::cfg_if! {
                if #[cfg(target_os = "macos")] {
                    fastab_integrations::input_method::InputMethod::default().uninstall().await?;
                    uninstalled = true;
                    Ok(())
                } else {
                    Err(eyre::eyre!("Input method integration is only supported on macOS"))
        ...
        Integration::AutostartEntry => {
            cfg_if::cfg_if! {
                if #[cfg(target_os = "linux")] {
                    use fastab_integrations::desktop_entry::AutostartIntegration;
...
async fn status(integration: Integration, format: OutputFormat) -> Result<ExitCode> {
    match integration {
        ...
        Integration::InputMethod => {
            cfg_if::cfg_if! {
                if #[cfg(target_os = "macos")] {
                    let input_method = fastab_integrations::input_method::InputMethod::default();
