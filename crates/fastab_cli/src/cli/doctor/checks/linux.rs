...
use fastab_ipc::local::send_recv_command_to_socket;
use fastab_os_shim::Context;
use fastab_proto::local::command::Command as IpcCommand;
use fastab_proto::local::command_response::Response;
use fastab_proto::local::{CommandResponse, ConnectToIBusCommand};
use fastab_util::Terminal;
use fastab_util::consts::{CLI_BINARY_NAME, PRODUCT_NAME};
use fastab_util::system_info::linux::{DesktopEnvironment, DisplayServer, get_desktop_environment, get_display_server};
...
impl DoctorCheck<LinuxContext> for DisplayServerCheck {
    ...
    async fn check(&self, ctx: &LinuxContext) -> Result<(), DoctorError> {
        match get_display_server(&ctx.ctx) {
            Ok(_) => Ok(()),
            Err(fastab_util::Error::UnknownDisplayServer(server)) => Err(doctor_error!(
...
impl DoctorCheck<LinuxContext> for SandboxCheck {
    ...
    async fn check(&self, _: &LinuxContext) -> Result<(), DoctorError> {
        ...
        let kind = fastab_util::system_info::linux::detect_sandbox();
...
mod tests {
    ...
    use fastab_os_shim::{Env, ProcessInfo};
