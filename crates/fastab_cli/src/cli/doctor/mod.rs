...
#[cfg(target_os = "macos")]
use fastab_integrations::input_method::InputMethodError;
use fastab_integrations::shell::{ShellExt, ShellIntegration};
use fastab_integrations::ssh::SshIntegration;
use fastab_integrations::{Error as InstallationError, Integration};
use fastab_ipc::{BufferedUnixStream, SendMessage, SendRecvMessage};
use fastab_os_shim::{Context, Env, Os};
use fastab_proto::local::DiagnosticsResponse;
use fastab_settings::JsonStore;
use fastab_util::directories::{remote_socket_path, settings_path};
use fastab_util::env_var::{PROCESS_LAUNCHED_BY_Q, Q_PARENT, QTERM_SESSION_ID};
use fastab_util::macos::BUNDLE_CONTENTS_INFO_PLIST_PATH;
use fastab_util::system_info::SupportLevel;
use fastab_util::terminal::in_special_terminal;
use fastab_util::{
...
use crate::util::desktop::{LaunchArgs, desktop_app_running, launch_fastab_desktop};
...
impl From<fastab_util::Error> for DoctorError {
    fn from(err: fastab_util::Error) -> Self {
        DoctorError::Error {
            reason: err.to_string().into(),
            info: vec![],
            fix: None,
...
async fn check_socket_perms(path: impl AsRef<Path>) -> Result<(), DoctorError> {
    ...
    fastab_ipc::validate_socket(&path)
        .await
...
impl DoctorCheck for FigBinCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        let path = directories::fastab_data_dir().map_err(eyre::Report::from)?;
...
impl DoctorCheck for RemoteSocketCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        let q_parent = fastab_os_shim::Env::new().q_parent().ok();
...
impl DoctorCheck for SettingsCorruptionCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        fastab_settings::OldSettings::load().map_err(|_err| DoctorError::Error {
...
impl DoctorCheck for FigIntegrationsCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        ...
        if let Some(exe) = fastab_util::get_parent_process_exe() {
        ...
        if fastab_util::system_info::in_ci() {
            return Err(DoctorError::Error {
        ...
        // TODO(grant): Check fastabterm exe exists
        // let fastabterm_path = fig_directories::fig_dir()
        //    .context("Could not find ~/.fig")?
        //    .join("bin")
        //    .join("fastabterm");

        // if !fastabterm_path.exists() {
        //    return Err(DoctorError::Error {
        //        reason: "fastabterm does not exist".into(),
        //        info: vec![],
        //        fix: None,
        //    });
        //}

        match fastab_os_shim::Env::new().q_term().as_deref() {
...
impl DoctorCheck for PtySocketCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        ...
        let socket_path = fastab_util::directories::fastabterm_socket_path(term_session).context("No qterm path")?;
        ...
        let write_handle: tokio::task::JoinHandle<Result<BufferedUnixStream, DoctorError>> = tokio::spawn(async move {
            ...
            let message = fastab_proto::figterm::FigtermRequestMessage {
                request: Some(fastab_proto::figterm::figterm_request_message::Request::InsertText(
                    fastab_proto::figterm::InsertTextRequest {
        ...
        let message = fastab_proto::figterm::FigtermRequestMessage {
            request: Some(fastab_proto::figterm::figterm_request_message::Request::Diagnostics(
                fastab_proto::figterm::DiagnosticsRequest {},
            )),
        };

        let response: Result<Option<fastab_proto::figterm::FigtermResponseMessage>> = conn
            .send_recv_message_timeout(message, Duration::from_secs(1))
            .await
            .context("Failed to send/recv message");

        match response {
            Ok(Some(fastabterm_response)) => match fastabterm_response.response {
                Some(fastab_proto::figterm::figterm_response_message::Response::Diagnostics(
                    fastab_proto::figterm::DiagnosticsResponse {
                    ...
                    if let Some(style) = zsh_autosuggestion_style {
                        if let Some(fg) = style.fg {
                            if let Some(fastab_proto::figterm::term_color::Color::Indexed(i)) = fg.color {
                                if i == 15 {
...
impl DoctorCheck<DiagnosticsResponse> for ShellCompatibilityCheck {
    ...
    async fn check(&self, _: &DiagnosticsResponse) -> Result<(), DoctorError> {
        ...
        let current_shell = fastab_util::get_parent_process_exe();
...
impl DoctorCheck<DiagnosticsResponse> for AutocompleteEnabledCheck {
    ...
    async fn check(&self, _diagnostics: &DiagnosticsResponse) -> Result<(), DoctorError> {
        if !fastab_settings::settings::get_bool_or("autocomplete.disable", false) {
            Ok(())
        } else {
            Err(DoctorError::Error {
...
macro_rules! dev_mode_check {
    ($struct_name:ident, $check_name:expr, $settings_module:ident, $setting_name:expr) => {
        ...
        impl DoctorCheck for $struct_name {
            ...
            async fn check(&self, _: &()) -> Result<(), DoctorError> {
                if let Ok(Some(true)) = fastab_settings::$settings_module::get_bool($setting_name) {
...
impl DoctorCheck<SupportedTerminalCheckContext> for SupportedTerminalCheck {
    ...
    async fn get_type(&self, _: &SupportedTerminalCheckContext, platform: Platform) -> DoctorCheckType {
        if fastab_util::system_info::is_remote() {
            DoctorCheckType::NoCheck
        } else {
            match platform {
...
impl DoctorCheck for SystemVersionCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        let os_version = fastab_util::system_info::os_version().wrap_err("Could not get OS Version")?;
...
impl DoctorCheck<SupportedTerminalCheckContext> for ImeStatusCheck {
    ...
    async fn check(&self, _context: &SupportedTerminalCheckContext) -> Result<(), DoctorError> {
        use fastab_integrations::Integration;
        ...
        if let Err(e) = input_method.is_installed().await {
            fastab_settings::state::set_value("input-method.enabled", true).ok();
...
impl DoctorCheck for DesktopCompatibilityCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        ...
        use fastab_util::system_info::linux::{
...
impl DoctorCheck for DashboardHostCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        match fastab_settings::settings::get_string("developer.dashboard.host")
            .ok()
            .flatten()
        {
            Some(host) => {
...
impl DoctorCheck for AutocompleteHostCheck {
    ...
    async fn check(&self, _: &()) -> Result<(), DoctorError> {
        match fastab_settings::settings::get_string("developer.autocomplete.host")
            .ok()
            .flatten()
        {
            Some(host) => {
...
pub async fn doctor_cli(all: bool, strict: bool) -> Result<ExitCode> {
    ...
    if let Ok(update_lock) = fastab_util::directories::update_lock_path(&Context::new()) {
    ...
    let shell_integrations: Vec<_> = [Shell::Bash, Shell::Zsh, Shell::Fish]
        ...
        .collect::<Result<Vec<_>, fastab_integrations::Error>>()?
        .into_iter()
        .flatten()
    ...
    let status: Result<()> = async {
        ...
        if fastab_util::manifest::is_full() {
            run_checks(
        ...
        if fastab_util::manifest::is_minimal() {
            return Ok(());
        }
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if fastab_util::manifest::is_full() && !fastab_util::system_info::is_remote() {
                run_checks_with_context(
    ...
    if fastab_settings::state::get_bool_or("doctor.prompt-restart-terminal", false) {
        println!(
            "  {}{}",
        ...
        fastab_settings::state::set_value("doctor.prompt-restart-terminal", false)?;
    }
