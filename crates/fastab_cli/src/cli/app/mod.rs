...
use fastab_ipc::local::update_command;
use fastab_settings::{settings, state};
use fastab_util::{CLI_BINARY_NAME, PRODUCT_NAME, manifest};
...
pub async fn restart_fig() -> Result<()> {
    if fastab_util::system_info::in_cloudshell() {
   ...
   * {PRODUCT_NAME} not working? Run {}
            ...
            AppSubcommand::Prompts => {
                if fastab_util::manifest::is_minimal() {
                    ...
                    if !no_autolaunch && !user_quit_app && !fastab_util::system_info::in_ssh() {
                        let already_seen_hint: bool =
                            fastab_settings::state::get_bool_or("DISPLAYED_AUTOLAUNCH_SETTINGS_HINT", false);
                        ...
                        if !already_seen_hint {
                            ...
                            fastab_settings::state::set_value("DISPLAYED_AUTOLAUNCH_SETTINGS_HINT", true)?;
                        }
            ...
            AppSubcommand::Uninstall(args) => {
                if !args.only_open {
                    fastab_install::uninstall(args.into(), fastab_os_shim::Context::new()).await?;
                }
            },
