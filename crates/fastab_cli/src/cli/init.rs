...
use eyre::Result;
use fastab_integrations::shell::{ShellExt, When};
use fastab_os_shim::{Context, Env};
use fastab_util::env_var::Q_SHELL;
use fastab_util::{CLI_BINARY_NAME, PRODUCT_NAME, Shell, Terminal, get_parent_process_exe};
use indoc::formatdoc;

use super::internal::should_fastabterm_launch::should_fastabterm_launch_exit_status;
...
static IS_SNAPSHOT_TEST: LazyLock<bool> = LazyLock::new(|| fastab_os_shim::Env::new().q_init_snapshot_test());
...
async fn shell_init(shell: &Shell, when: &When, rcfile: &Option<String>) -> Result<String> {
    ...
    if !fastab_settings::state::get_bool_or(SHELL_INTEGRATIONS_ENABLED_STATE_KEY, true) {
    ...
    if when == &When::Pre {
        let status = if *IS_SNAPSHOT_TEST {
            ...
            should_fastabterm_launch_exit_status(&Context::new(), true)
        };
    ...
    if let When::Post = when {
        ...
        //     let has_seen_onboarding: bool = fastab_settings::state::get_bool_or("user.onboarding", true);
        ...
        if fastab_settings::state::get_bool_or("shell-integrations.immediateLogin", false)
            && fastab_settings::state::set_value("shell-integrations.immediateLogin", false).is_ok()
        {
    ...
    if let Some(path) = fastab_settings::settings::get_string_opt("qterm.path") {
    ...
    if when == &When::Pre && is_jetbrains_terminal {
        // Manually call JetBrains shell integration after exec-ing to fastabterm.
    ...
    #[cfg(target_os = "macos")]
    if when == &When::Post
        && !fastab_integrations::input_method::InputMethod::default()
            .is_enabled()
            .unwrap_or(false)
    {
    ...
    if when == &When::Post && !fastab_settings::state::get_bool_or("desktop.auth-watcher.logged-in", true) {
        let last_sent_at = fastab_settings::state::get_int_or("cli.init.login-prompt.sent-at", 0);
        let now = SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs() as i64;
        if now - last_sent_at > 60 * 60 * 36 {
            let _ = fastab_settings::state::set_value("cli.init.login-prompt.sent-at", now);
