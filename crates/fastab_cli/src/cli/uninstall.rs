...
pub async fn uninstall_command(no_confirm: bool) -> Result<ExitCode> {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            use fastab_util::manifest::is_minimal;
            if is_minimal() {
...
#[cfg(target_os = "macos")]
async fn uninstall(ctx: std::sync::Arc<fastab_os_shim::Context>) -> Result<()> {
    fastab_install::uninstall(fastab_install::InstallComponents::all(), ctx).await?;
    Ok(())
}

#[cfg(target_os = "linux")]
async fn uninstall_linux_minimal(ctx: std::sync::Arc<fastab_os_shim::Context>) -> Result<()> {
    use eyre::bail;
    use tracing::error;
    ...
    let local_bin = fastab_util::directories::home_local_bin_ctx(&ctx)?.canonicalize()?;
    ...
    fastab_install::uninstall(fastab_install::InstallComponents::all_linux_minimal(), ctx).await?;
    Ok(())
}

#[cfg(target_os = "linux")]
async fn uninstall_linux_full(ctx: std::sync::Arc<fastab_os_shim::Context>) -> Result<()> {
    use eyre::bail;
    use fastab_install::{InstallComponents, uninstall};
