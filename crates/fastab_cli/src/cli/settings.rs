...
use fastab_ipc::local::open_ui_element;
use fastab_os_shim::Os;
use fastab_proto::local::UiElement;
use fastab_settings::JsonStore;
use fastab_util::{CLI_BINARY_NAME, directories, manifest, system_info};
use globset::Glob;
use serde_json::json;
    ...
    pub async fn execute(&self, cli_context: &CliContext) -> Result<ExitCode> {
        ...
        match self.cmd {
            ...
            Some(SettingsSubcommands::List { format }) => {
                let settings = fastab_settings::OldSettings::load()?;
            ...
            None => match &self.key {
                Some(key) => match (&self.value, self.delete) {
                    ...
                    (None, false) => match fastab_settings::settings::get_value(key)? {
                        Some(value) => {
                            match self.format {
                    ...
                    (Some(value_str), false) => {
                        ...
                        fastab_settings::settings::set_value(key, value)?;
                        Ok(ExitCode::SUCCESS)
                    },
                    (None, true) => {
                        ...
                        match keys_to_remove.len() {
                            ...
                            1 => {
                                ...
                                fastab_settings::settings::remove_value(keys_to_remove[0])?;
                            },
                            _ => {
                                println!("Removing:");
                                ...
                                for key in &keys_to_remove {
                                    fastab_settings::settings::remove_value(key)?;
                                }
                            },
                        }
