...
use fastab_ipc::Base64LineCodec;
use fastab_proto::figterm::intercept_request::{InterceptCommand, SetFigjsIntercepts, SetFigjsVisible};
use fastab_proto::figterm::{InsertTextRequest, InterceptRequest, SetBufferRequest};
use fastab_proto::local::{EditBufferHook, InterceptedKeyHook, PostExecHook, PreExecHook, PromptHook};
use fastab_proto::mux::{self, Packet, PacketOptions, Ping, message_to_packet, packet_to_message};
use fastab_proto::remote;
use fastab_proto::remote::RunProcessRequest;
use fastab_remote_ipc::figterm::{FigtermCommand, FigtermState};
use fastab_remote_ipc::remote::handle_remote_ipc;
use fastab_util::{PTY_BINARY_NAME, directories};
...
pub async fn execute(args: MultiplexerArgs) -> Result<()> {
    #[cfg(unix)]
    let pid_lock = match fastab_util::directories::runtime_dir() {
    ...
    let fastabterm_state = Arc::new(FigtermState::new());
    ...
    loop {
        let control_flow = select! {
            stream = listener.accept() => {
                match stream {
                    Ok((stream, _)) => {
                        ...
                        tokio::spawn(handle_remote_ipc(stream, fastabterm_state.clone(), SimpleHookHandler {
            ...
            packet = reader.next() => forward_packet_to_fastabterm(packet, &fastabterm_state, &host_sender).await,
...
async fn forward_packet_to_fastabterm(
    ...
    match packet {
        Some(Ok(packet)) => {
            ...
            match handle_clientbound(message, fastabterm_state, host_sender).await {
                Ok(Some((msg, session_id))) => {
                    if let Some(session) = fastabterm_state.get(&session_id) {
...
async fn handle_clienbound_request(
    ...
    Ok(match inner {
        ...
        mux::clientbound::request::Inner::RunProcess(RunProcessRequest {
            ...
            sender.send(command).context("Failed sending command to fastabterm")?;
            drop(sender);

            let timeout_duration = timeout.unwrap_or(Duration::from_secs(60));
            let response = tokio::time::timeout(timeout_duration, rx)
                .await
                .context("Timed out waiting for fastabterm response")?
                .context("Failed to receive fastabterm response")?;
...
impl fastab_remote_ipc::RemoteHookHandler for SimpleHookHandler {
    ...
    async fn edit_buffer(
        ...
        session_id: Uuid,
        _fastabterm_state: &Arc<FigtermState>,
...
#[cfg(test)]
mod tests {
    use fastab_proto::fig::ShellContext;
    use uuid::Uuid;
