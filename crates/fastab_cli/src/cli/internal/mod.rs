...
pub mod local_state;
mod multiplexer;
pub mod should_fastabterm_launch;
...
use fastab_install::InstallComponents;
#[cfg(target_os = "macos")]
use fastab_integrations::input_method::InputMethod;
use fastab_ipc::local::send_hook_to_socket;
use fastab_ipc::{BufferedUnixStream, SendMessage, SendRecvMessage};
use fastab_os_shim::{Context as OsContext, Os};
use fastab_proto::ReflectMessage;
use fastab_proto::figterm::figterm_request_message::Request as FigtermRequest;
use fastab_proto::figterm::{FigtermRequestMessage, NotifySshSessionStartedRequest, UpdateShellContextRequest};
use fastab_proto::hooks::{new_callback_hook, new_event_hook};
use fastab_proto::local::EnvironmentVariable;
use fastab_proto::util::get_shell;
use fastab_util::directories::{fastabterm_socket_path, logs_dir, update_lock_path};
use fastab_util::env_var::QTERM_SESSION_ID;
use fastab_util::{CLI_BINARY_NAME, directories};
...
use crate::util::desktop::{LaunchArgs, launch_fastab_desktop};
...
pub enum InternalSubcommand {
    ...
    ///
    /// Exit code:
    /// - 0 execute fastabterm
    /// - 1 dont execute fastabterm
    ...
    pub async fn execute(self) -> Result<ExitCode> {
        ...
        match self {
            ...
            InternalSubcommand::Uninstall {
                ...
                if components.contains(InstallComponents::BINARY) {
                    if option_env!("Q_IS_PACKAGE_MANAGED").is_some() {
                        ...
                        fastab_install::uninstall(InstallComponents::BINARY, Arc::clone(&ctx)).await?;
                ...
                fastab_install::uninstall(components, Arc::clone(&ctx)).await?;
                Ok(ExitCode::SUCCESS)
            },
            ...
            InternalSubcommand::ShouldFigtermLaunch => {
                Ok(should_fastabterm_launch::should_fastabterm_launch(&OsContext::new()))
            },
            ...
            InternalSubcommand::Ipc {
                ...
                let message = fastab_proto::FigMessage::json(serde_json::from_str::<serde_json::Value>(&json)?)?;

                let socket = if app {
                    directories::desktop_socket_path().expect("Failed to get socket path")
                } else if let Some(ref fastabterm) = fastabterm {
                    directories::fastabterm_socket_path(fastabterm).expect("Failed to get socket path")
                } else {
                ...
                if recv {
                    ...
                    if app {
                        recv!(fastab_proto::local::CommandResponse);
                    } else if fastabterm.is_some() {
                        recv!(fastab_proto::figterm::FigtermResponseMessage);
                    }
                } else {
            ...
            InternalSubcommand::FigtermSocketPath { session_id } => {
                writeln!(
                    stdout(),
                    "{}",
                    directories::fastabterm_socket_path(session_id)?.to_string_lossy()
                )
                .ok();
                Ok(ExitCode::SUCCESS)
            },
            InternalSubcommand::UninstallForAllUsers => {
                ...
                // let emit = tokio::spawn(fastab_telemetry::emit_track(TrackEvent::new(
            ...
            InternalSubcommand::DetectSandbox => {
                use fastab_util::system_info::linux::SandboxKind;
                let exit_code = match fastab_util::system_info::linux::detect_sandbox() {
                    SandboxKind::None => {
            ...
            InternalSubcommand::SshLocalCommand { remote_dest, uuid } => {
                ...
                if let Ok(session_id) = std::env::var(QTERM_SESSION_ID) {
                    let mut conn =
                        BufferedUnixStream::connect(fastab_util::directories::fastabterm_socket_path(&session_id)?)
                            .await?;
            ...
            InternalSubcommand::DumpState { component } => {
                use fastab_proto::local::dump_state_command::Type as StateCommandType;

                let state = fastab_ipc::local::dump_state_command(match component {
            ...
            InternalSubcommand::FinishUpdate {
                ...
                let _ = tokio::fs::remove_file(update_lock_path(&fastab_os_shim::Context::new())?).await;
            ...
            InternalSubcommand::SwapFiles {
                ...
                match fastab_install::macos::install(from_cstr, to_cstr, !not_same_bundle_name) {
                    Ok(_) => {
            ...
            InternalSubcommand::BrewUninstall { zap } => {
                ...
                fastab_install::uninstall(components, fastab_os_shim::Context::new())
                    .await
                    .ok();
                Ok(ExitCode::SUCCESS)
            },
...
pub async fn pre_cmd(alias: Option<String>) -> ExitCode {
    ...
    match fastabterm_socket_path(&session_id) {
        Ok(fastabterm_path) => match fastab_ipc::socket_connect(fastabterm_path).await {
            Ok(mut fastabterm_stream) => {
                ...
                if let Err(err) = fastabterm_stream.send_message(message).await {
...
async fn remove_data_dir(ctx: Arc<OsContext>, force: bool) -> Result<ExitCode> {
    ...
    let data_dir = directories::fastab_data_dir_ctx(&ctx)?;
...
mod tests {
    ...
    async fn test_remove_data_dir() {
        ...
        let fs = ctx.fs();
        let data_dir = directories::fastab_data_dir_ctx(&ctx).unwrap();
