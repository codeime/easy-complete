...
#[allow(dead_code)]
struct ProcessInfo {
    pid: Box<fastab_os_shim::process_info::Pid>,
    exe_name: String,
...
fn parent_status(ctx: &Context, current_pid: fastab_os_shim::process_info::Pid) -> Status {
    use fastab_util::env_var::Q_TERM;
    let env = ctx.env();
...
fn grandparent_status(ctx: &Context, parent_pid: fastab_os_shim::process_info::Pid) -> Status {
    ...
    let terminals = match current_os {
        fastab_os_shim::Os::Mac => fastab_util::terminal::MACOS_TERMINALS,
        fastab_os_shim::Os::Linux => fastab_util::terminal::LINUX_TERMINALS,
        _ => panic!("unsupported os"),
    }
    .iter()
    .chain(fastab_util::terminal::SPECIAL_TERMINALS)
    .cloned()
    .collect::<Vec<_>>();
    ...
    let valid_grandparent = match current_os {
        fastab_os_shim::Os::Mac => terminals
            .iter()
            .find(|terminal| terminal.executable_names().contains(&grandparent_name)),
        fastab_os_shim::Os::Linux => terminals.iter().find(|terminal| {
...
#[cfg(any(target_os = "macos", target_os = "linux"))]
pub fn should_fastabterm_launch_exit_status(ctx: &Context, quiet: bool) -> u8 {
    use fastab_util::env_var::{PROCESS_LAUNCHED_BY_Q, Q_PARENT};
...
#[cfg(any(target_os = "macos", target_os = "linux"))]
pub fn should_fastabterm_launch(ctx: &Context) -> ExitCode {
    ExitCode::from(should_fastabterm_launch_exit_status(ctx, false))
}
...
mod tests {
    ...
    use fastab_os_shim::process_info::{FakePid, Pid, ProcessInfo};
    use fastab_os_shim::{ContextBuilder, Env, Os, Platform};
    use fastab_util::env_var::{PROCESS_LAUNCHED_BY_Q, Q_PARENT, Q_TERM};
    ...
    macro_rules! assert_exit_code {
        ($ctx:expr, $exit_code:expr, $msg:expr) => {
            assert_eq!(
                should_fastabterm_launch_exit_status(&$ctx, true),
                $exit_code,
                "{}",
                $msg
            );
            assert_eq!(
                should_fastabterm_launch_exit_status(&$ctx, false),
                $exit_code,
                "{}",
                $msg
            );
