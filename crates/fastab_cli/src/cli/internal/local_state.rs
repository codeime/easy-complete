...
use fastab_ipc::local::restart_settings_listener;
    ...
    pub async fn execute(&self) -> Result<()> {
        ...
        match self.cmd {
            ...
            Some(LocalStateSubcommand::All { format }) => {
                let state = fastab_settings::state::all()?;
                match format {
                    OutputFormat::Plain => {
            ...
            None => match &self.key {
                Some(key) => match (&self.value, self.delete) {
                    (None, false) => match fastab_settings::state::get_value(key)? {
                        Some(value) => {
                            match self.format {
