...
use fastab_install::{InstallComponents, install};
...
pub async fn install_cli(
    ...
    if install_components.contains(InstallComponents::SHELL_INTEGRATIONS) && !global {
        ...
        if !no_confirm && manual_install {
            let shell_dir = fastab_util::directories::fastab_data_dir_utf8()?.join("shell");
            let shell_dir = shell_dir
                .strip_prefix(fastab_util::directories::home_dir()?)
                .unwrap_or(&shell_dir);
