...
use fastab_ipc::local::{devtools_command, prompt_accessibility_command, set_debug_mode, toggle_debug_mode};
...
use fastab_util::{APP_BUNDLE_NAME, CLI_BINARY_NAME, PRODUCT_NAME, PTY_BINARY_NAME, Shell, directories};
use owo_colors::OwoColorize;
use tempfile::{NamedTempFile, TempDir};
use tracing::error;

use crate::cli::launch_fastab_desktop;
...
#[cfg(target_os = "macos")]
use fastab_integrations::{Integration, input_method::InputMethod};
    ...
    pub async fn execute(&self) -> Result<ExitCode> {
        ...
        match self {
            ...
            DebugSubcommand::Build { build, app } => match build {
                Some(build) => {
                    fastab_settings::settings::set_value(
                ...
                None => {
                    let current_build = fastab_settings::settings::get_string_opt(format!("developer.{app}.build"));
            ...
            DebugSubcommand::Logs { level, files } => {
                ...
                fastab_settings::state::set_value("developer.logging", true)?;

                if files.is_empty() || files.iter().any(|f| f == "fastab_desktop") {
                    if let Err(err) =
                        fastab_ipc::local::set_log_level(level.as_ref().clone().unwrap_or_else(|| "DEBUG".into())).await
                    {
                        println!("Could not set log level for fastab_desktop: {err}");
                    }
                }

                tokio::spawn(async move {
                    tokio::signal::ctrl_c().await.unwrap();
                    let code = match fastab_settings::state::set_value("developer.logging", false) {
                        Ok(_) => 0,
                        Err(_) => 1,
                    };

                    if let Err(err) = fastab_ipc::local::set_log_level("INFO".into()).await {
                        println!("Could not restore log level for fastab_desktop: {err}");
                    }
                ...
                let log_paths = if files.is_empty() {
                    ...
                    if files.iter().any(|f| f == PTY_BINARY_NAME) {
                        // Remove fastabterm from the list of files to open
                        files.retain(|f| f != PTY_BINARY_NAME);

                        // Add fastabterm*.log to the list of files to open
                        ...
                        return match InputMethod::list_input_sources_for_bundle_id(bundle_identifier.as_str()) {
                            Some(sources) => {
                                sources
                                    ...
                                    .collect::<Result<Vec<()>, fastab_integrations::input_method::InputMethodError>>(
                                    )?;
                                Ok(ExitCode::SUCCESS)
                            },
            ...
            DebugSubcommand::VerifyCodesign => {
                Command::new("codesign")
                    .arg("-vvvv")
                    .arg(fastab_util::app_bundle_path())
                    .spawn()?
                    .wait()?;
            },
            ...
            Self::Devtools { app } => {
                ...
                devtools_command(match app {
                    App::Dashboard => fastab_proto::local::devtools_command::Window::DevtoolsDashboard,
                    App::Autocomplete => fastab_proto::local::devtools_command::Window::DevtoolsAutocomplete,
                })
                .await
