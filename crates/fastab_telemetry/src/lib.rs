...
/// `endpoint` and `api_key` are ignored. Reporting uses a placeholder host
/// and an empty API key, which disables sending.
///
...
pub fn init(endpoint: impl Into<String>, api_key: impl Into<String>) {
    ...
    let url = "https://telemetry.placeholder.invalid/capture/";
    let key = "";
...
fn is_enabled() -> bool {
    fastab_settings::settings::get_bool_or(TELEMETRY_ENABLED_KEY, false)
}
...
fn device_id() -> String {
    if let Ok(Some(id)) = fastab_settings::state::get_string(DEVICE_ID_KEY) {
        return id;
    }
    let id = Uuid::new_v4().to_string();
    fastab_settings::state::set_value(DEVICE_ID_KEY, id.as_str()).ok();
    id
}
...
fn os_version_string() -> String {
    fastab_util::system_info::os_version().map_or_else(|| "unknown".into(), |v| v.to_string())
}
...
fn terminal_name() -> String {
    fastab_util::terminal::Terminal::parent_terminal(&fastab_os_shim::Context::new())
        .map_or_else(|| "unknown".into(), |t| t.internal_id().into_owned())
}

fn queue_path() -> Option<PathBuf> {
    fastab_util::directories::fastab_data_dir()
        .ok()
...
fn http_client() -> Option<reqwest::Client> {
    ...
    reqwest::Client::builder()
        .user_agent(format!("{}/{}", fastab_util::consts::APP_PROCESS_NAME, app_version()))
...
fn build_event(event: &str, extra_props: Value) -> Value {
    let mut props = json!({
        "app_name":    fastab_util::consts::PRODUCT_NAME,
...
pub async fn maybe_send_daily_heartbeat() {
    ...
    let last = fastab_settings::state::get_int_or(LAST_HEARTBEAT_KEY, 0);
    let now = now_unix();
    ...
    if let Ok(map) = fastab_settings::state::all() {
        for (key, value) in map {
            ...
            fastab_settings::state::remove_value(key).ok();
        }
    }
    ...
    fastab_settings::state::set_value(LAST_HEARTBEAT_KEY, now).ok();
