...
use fastab_util::consts::{APP_BUNDLE_ID, APP_BUNDLE_NAME, APP_PROCESS_NAME, PRODUCT_NAME};
use fastab_util::launchd_plist::{LaunchdPlist, create_launch_agent};
...
/// historical LaunchAgents so an upgrade cannot leave duplicate jobs behind.
...
fn sm_error_message(error: *mut Object, action: &str) -> String {
    if error.is_null() {
        return format!("Failed to {action} the {PRODUCT_NAME} login item");
    }
    ...
    unsafe {
        ...
        format!(
            "Failed to {action} the {PRODUCT_NAME} login item: {}",
...
fn set_legacy_launch_agent_enabled(enabled: bool) -> Result<()> {
    ...
    if !status.success() {
        return Err(Error::Custom(
            format!("launchctl failed to load the {PRODUCT_NAME} LaunchAgent").into(),
        ));
    }
...
fn current_app_executable() -> Result<PathBuf> {
    ...
    let bundle = current
        ...
        .ok_or_else(|| Error::ApplicationNotInstalled(Cow::Borrowed(APP_BUNDLE_NAME)))?;
...
fn legacy_launch_agent_path(label: &str) -> Result<PathBuf> {
    Ok(fastab_util::directories::home_dir()?
        .join("Library")
        .join("LaunchAgents")
...
mod tests {
    ...
    fn macos_12_launch_agent_starts_silently() {
        let plist = legacy_launch_agent(PathBuf::from("/Applications/Fastab.app/Contents/MacOS/easy-complete")).plist();
