...
use fastab_util::directories::{self, fastab_data_dir_utf8, home_dir};
use regex::Regex;
...
impl SshIntegration {
    ...
    fn get_integration_path(&self) -> Result<PathBuf> {
        Ok(directories::fastab_data_dir()?.join(SSH_OUTER_NAME))
    }

    fn get_file_integration(&self) -> Result<FileIntegration> {
        let bin_name = CLI_BINARY_NAME;
        let include_path = fastab_data_dir_utf8()?.join(SSH_INNER_NAME);
...
impl Integration for SshIntegration {
    ...
    async fn install(&self) -> Result<()> {
        ...
        let mut contents = if self.path.exists() {
            backup_file(&self.path, fastab_util::directories::utc_backup_dir().ok())?;
    ...
    async fn uninstall(&self) -> Result<()> {
        ...
        let (res_1, res_2, res_3) = tokio::join!(self.uninstall_ssh_config(), file_integration.uninstall(), async {
            ...
            let _ = tokio::fs::remove_file(directories::fastab_data_dir()?.join(SSH_INNER_NAME)).await;
            Ok(())
        });
