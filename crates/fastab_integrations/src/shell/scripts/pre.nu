...
let result = (^{{CLI_BINARY_NAME}} _ should-fastabterm-launch | complete)
...
if $should_launch {
  ...
  let fig_term_name = "nu (fastabterm)"
  let fastabterm_path = if ([$env.HOME ".fig" "bin" $fig_term_name] | path join | path exists) {
    [$env.HOME ".fig" "bin" $fig_term_name] | path join
  } else if (which fastabterm | length) > 0 {
    which fastabterm | first | get path
  } else {
    [$env.HOME ".fig" "bin" "fastabterm"] | path join
  }

  with-env {
    Q_SHELL: $Q_SHELL
  } {
    exec $fastabterm_path
  }
}
