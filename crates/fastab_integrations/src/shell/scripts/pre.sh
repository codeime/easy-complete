...
if [ -z "${SHOULD_QTERM_LAUNCH:-}" ]; then
  {{CLI_BINARY_NAME}} _ should-fastabterm-launch 1>/dev/null 2>&1
  SHOULD_QTERM_LAUNCH=$?
fi

# Only launch fastabterm if current session is not already inside PTY and command exists.
...
then
  ...
  # Do not launch fastabterm in non-interactive shells (like VSCode Tasks)
  if [[ $- == *i* ]]; then
