...
pub enum SendError {
    #[error(transparent)]
    Encode(#[from] fastab_proto::FigMessageEncodeError),
    #[error(transparent)]
    Io(#[from] std::io::Error),
}
...
pub enum RecvError {
    ...
    Io(#[from] std::io::Error),
    #[error(transparent)]
    Parse(#[from] fastab_proto::FigMessageParseError),
    #[error(transparent)]
    Decode(#[from] fastab_proto::FigMessageDecodeError),
...
mod tests {
    ...
    fn is_disconnect() {
        assert!(
            !RecvError::Decode(fastab_proto::FigMessageDecodeError::NameNotValid("test".to_string())).is_disconnect()
        );
