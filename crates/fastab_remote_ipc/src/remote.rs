...
pub async fn start_remote_ipc(
    ...
    while let Ok((stream, _)) = listener.accept().await {
        tokio::spawn(handle_remote_ipc(stream, fastabterm_state.clone(), hook.clone()));
    }
...
pub async fn handle_remote_ipc(
                ...
                Ok(Some(message)) => {
                    ...
                    if let Some(response) = match message.packet {
                        Some(hostbound::Packet::Handshake(handshake)) => {
                            let result = if initialized {
                            ...
                            } else if let Some(success) = fastabterm_state.with_update(session_id, |session| {
                                ...
                                tokio::spawn(handle_commands(command_rx, fastabterm_state.clone(), session_id));
                                debug!(
                                    "Client auth for {} accepted because of new id with secret {}",
                                    handshake.id, handshake.secret
                                );
                                fastabterm_state.insert(FigtermSession {
                                    id: session_id,
                            ...
                            if matches!(result, Some(clientbound::Packet::HandshakeResponse(HandshakeResponse { success: true }))) {
                                hook.sessions_changed(&fastabterm_state).await;

                                if let Some(parent_id) = handshake.parent_id {
                                    let inner = fastabterm_state.inner.lock();
                                ...
                                */
                                let res = match request {
                                    ...
                                    hostbound::request::Request::Prompt(mut prompt) => {
                                        ...
                                        hook.prompt(&prompt, session_id, &fastabterm_state).await
                                    },
                                    hostbound::request::Request::PreExec(mut pre_exec) => {
                                        ...
                                        hook.pre_exec(&pre_exec, session_id, &fastabterm_state).await
                                    },
                                    hostbound::request::Request::PostExec(mut post_exec) => {
                                        ...
                                        hook.post_exec(&post_exec, session_id, &fastabterm_state).await
                                    },
                        ...
                        Some(hostbound::Packet::Pong(())) => {
                            ...
                            fastabterm_state.with(&session_id, |session| {
    ...
    // fastabterm_state.with_update(session_id.clone(), |session| {
    //     session.writer = None;
    //     session.dead_since = Some(Instant::now());
    // });
    if fastabterm_state.remove_id(&session_id).is_some() {
        hook.sessions_changed(&fastabterm_state).await;
    }
...
async fn handle_commands(
    ...
    while let Ok(command) = incoming.recv_async().await {
        ...
        let nonce = if let Some(channel) = nonce_channel {
            Some(fastabterm_state.with(&session_id, |session| {
