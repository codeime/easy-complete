...
pub struct InnerFigtermState {
    ...
    /// The most recent fastabterm session
...
impl FigtermState {
    ...
    pub fn insert(&self, session: FigtermSession) {
        let mut fastabterm_state = self.inner.lock();
        fastabterm_state.most_recent = Some(session.id);
        fastabterm_state.linked_sessions.insert(session.id, session);
    }
...
pub enum FigtermCommand {
    InterceptFigJs {
        ...
        actions: Vec<fastab_proto::figterm::Action>,
