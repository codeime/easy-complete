...
/// Other terminals that fastabterm should launch within that are not full terminal emulators
...
impl Terminal {
    ...
    pub fn parent_terminal(ctx: &Context) -> Option<Self> {
        ...
        let terminals = match ctx.platform().os() {
            fastab_os_shim::Os::Mac => MACOS_TERMINALS,
            fastab_os_shim::Os::Linux => LINUX_TERMINALS,
            _ => return None,
        };
...
mod tests {
    ...
    use fastab_os_shim::process_info::TestExe;
    use fastab_os_shim::{Os, ProcessInfo};
