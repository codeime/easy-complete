...
use fastab_os_shim::{Context, EnvProvider, FsProvider, Os, PlatformProvider, Shim};
use thiserror::Error;
...
pub fn old_fastab_data_dir() -> Result<PathBuf> {
...
pub fn fastab_data_dir() -> Result<PathBuf> {
    Ok(dirs::data_local_dir()
        .ok_or(DirectoryError::NoHomeDirectory)?
        .join(DATA_DIR_NAME))
}

pub fn fastab_data_dir_ctx(fs: &impl FsProvider) -> Result<PathBuf> {
    Ok(fs.fs().chroot_path(fastab_data_dir()?))
}
...
pub fn remote_socket_path() -> Result<PathBuf> {
    ...
    if is_remote() && !in_cloudshell() && cfg!(target_os = "linux") {
        if let Some(parent_socket) = fastab_os_shim::Env::new().get_os(Q_PARENT) {
...
/// Get path to a fastabterm socket
///
/// - Linux/Macos: `/var/tmp/fig/%USERNAME%/fastabterm/$SESSION_ID.sock`
...
pub fn fastabterm_socket_path(session_id: impl Display) -> Result<PathBuf> {
...
pub fn resources_path() -> Result<PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(all(unix, not(target_os = "macos")))] {
            ...
            Ok(fastab_data_dir()?.join("resources"))
        }
    }
}
...
pub fn resources_path_ctx<Ctx: EnvProvider + PlatformProvider>(ctx: &Ctx) -> Result<PathBuf> {
    ...
    match os {
        fastab_os_shim::Os::Mac => Ok(crate::app_bundle_path().join(crate::macos::BUNDLE_CONTENTS_RESOURCE_PATH)),
        fastab_os_shim::Os::Linux => {
        ...
        fastab_os_shim::Os::Windows => Ok(fastab_data_dir()?.join("resources")),
...
pub fn settings_path() -> Result<PathBuf> {
    Ok(fastab_data_dir()?.join("settings.json"))
}
...
pub fn update_lock_path(ctx: &impl FsProvider) -> Result<PathBuf> {
    Ok(fastab_data_dir_ctx(ctx)?.join("update.lock"))
}
...
utf8_dir!(fastab_data_dir);
utf8_dir!(sockets_dir);
utf8_dir!(remote_socket_path);
utf8_dir!(fastabterm_socket_path, session_id: impl Display);
...
mod tests {
    ...
    fn socket_path_length() {
        ...
        let qterm_socket = fastabterm_socket_path(uuid.clone()).unwrap();
