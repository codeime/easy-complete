    ...
    fn grow_columns(&mut self, reflow: bool, columns: usize) {
        ...
        for (i, mut row) in rows.drain(..).enumerate().rev() {
            ...
            if let Some(cell) = last_row.last_mut() {
                cell.flags_mut().remove(ShellFlags::WRAPLINE);
            }
            ...
            let mut cells = if row[Column(len - 1)].flags().contains(ShellFlags::WIDE_CHAR) {
                num_wrapped -= 1;
            ...
            if let Some(cell) = last_row.last_mut() {
                ...
                cell.flags_mut().insert(ShellFlags::WRAPLINE);
            }
            ...
            loop {
                ...
                if len > 0 && wrapped[len - 1].flags().contains(ShellFlags::LEADING_WIDE_CHAR_SPACER) {
                    if len == 1 {
                        ...
                        wrapped[len - 2].flags_mut().insert(ShellFlags::WRAPLINE);
