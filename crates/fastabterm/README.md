...
First, make sure shell integrations are installed. You can use `fastab_cli` to do
this:
...
The shell integrations will then launch fastabterm on each terminal session.

You can verify fastabterm is running by:
...
roughly a process that a shell reads from and writes to. In fastabterm, this parent
process:
...
With fastabterm, we intercept the shell/terminal emulator communication:
`iTerm <-> fastabterm <-> zsh`. The fastabterm process looks like a shell to iTerm (it
...
Our `post` [shell integrations](../fastab_integrations/src/shell/scripts/) add
hooks that print custom OSC ANSI escape codes that `fastabterm` also parses. This
is our mechanism for sending information from the shell to `fastabterm`. We send
these codes to `fastabterm` to:
