...
use fastab_proto::figterm::{self, FigtermRequestMessage, FigtermResponseMessage, TelemetryRequest};
use fastab_proto::remote::{Clientbound, Hostbound, clientbound, hostbound};
use fastab_util::env_var::PROCESS_LAUNCHED_BY_Q;
use flume::Sender;
...
/// Process the inner fastabterm request enum, shared between local and remote
pub async fn process_fastabterm_request(
    fastabterm_request: FigtermRequest,
    ...
    match fastabterm_request {
        ...
        FigtermRequest::SetBuffer(_) => Err(anyhow::anyhow!("SetBuffer is not supported in fastabterm")),
...
/// Process a fastabterm request message
#[allow(clippy::too_many_arguments)]
pub async fn process_fastabterm_message(
    ...
    match figterm_request_message.request {
        ...
        Some(request) => {
            match process_fastabterm_request(request, main_loop_tx, term, pty_master, key_interceptor).await {
                Ok(Some(response)) => {
                ...
                Ok(None) => {},
                Err(err) => error!(%err, "Failed to process fastabterm message"),
            }
        },
...
async fn send_fastabterm_response_hostbound(
        ...
        Some(clientbound::Packet::Request(request)) => {
            ...
            match request.request {
                Some(Request::InsertText(request)) => {
                    send_fastabterm_response_hostbound(
                        process_fastabterm_request(
                            FigtermRequest::InsertText(request),
                            main_loop_tx,
                            term,
                            pty_master,
