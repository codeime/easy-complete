use fastab_settings::history::{HistoryColumn, Order, OrderBy, WhereExpression};
use flume::Sender;
...
pub enum HistoryCommand {
    ...
    Query(
        HistoryQueryParams,
        Sender<Option<Vec<fastab_settings::history::CommandInfo>>>,
    ),
}
        ...
        while let Ok(command) = receiver.recv_async().await {
            match command {
                HistoryCommand::Insert(command) => {
                    let command_info = fastab_settings::history::CommandInfo {
