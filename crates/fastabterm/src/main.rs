...
use crate::ipc::{spawn_fastabterm_ipc, spawn_remote_ipc};
use crate::message::{process_fastabterm_message, process_remote_message};
#[cfg(unix)]
    ...
    T: EventListener,
{
    let shell_enabled = ["bash", "zsh", "fish", "nu", "dash"]
        ...
        .any(|s| {
            ...
            // we actually want to work with a nested fastabterm :)
            let shell = match shell_raw.and_then(|s| s.strip_suffix(" (fastabterm)")) {
                Some(s) => Some(s),
                None => shell_raw,
            };
...
fn fastabterm_main(command: Option<&[String]>) -> Result<()> {
    ...
    let runtime_result = runtime.block_on(async {
        ...
        // Spawn thread to handle fastabterm ipc
        let incoming_receiver = spawn_fastabterm_ipc(&session_id).await?;
        ...
        let result: Result<()> = 'select_loop: loop {
            ...
            let select_result: Result<()> = select! {
                ...
                msg = incoming_receiver.recv_async() => {
                    match msg {
                        Ok((message, sender)) => {
                            debug!("Received message from fastabterm listener: {message:?}");
                            process_fastabterm_message(
                                message,
                                main_loop_tx.clone(),
                                sender.clone(),
                                &term,
