...
window are GPUI views. Completions run in `fastab_engine` on a worker thread.
...
- `crates/fastab_gpui` — list rendering, theme, AppKit frame
- `crates/fastab_engine` — IR lookup, generators, QuickJS hooks
