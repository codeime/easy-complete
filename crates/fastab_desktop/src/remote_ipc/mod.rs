...
impl fastab_remote_ipc::RemoteHookHandler for RemoteHook {
    ...
    async fn sessions_changed(&mut self, _fastabterm_state: &Arc<FigtermState>) {}
    ...
    async fn edit_buffer(
        ...
        let _old_metrics = fastabterm_state.with_update(session_id, |session| {
    ...
    async fn pre_exec(
        ...
        fastabterm_state.with_update(session_id, |session| {
            session.apply_context(hook.context.clone());
        });
