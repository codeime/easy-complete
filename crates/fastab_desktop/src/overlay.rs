...
use fastab_remote_ipc::figterm::{FigtermCommand, FigtermState, InterceptMode};
use fastab_settings::keybindings::{KeyBinding, KeyBindings};
...
pub struct OverlayController {
    ...
    enabled: bool,
    fastabterm_state: Arc<FigtermState>,
...
impl OverlayController {
    pub fn start(
        ...
        state.update(cx, |overlay, _cx| {
            ...
            overlay.on_disable_dev_mode = Some(Arc::new(move || {
                let _ = fastab_settings::settings::set_value("autocomplete.developerModeNPM", false);
    ...
    fn sync_own_intercept(&self, cx: &App) {
        self.sync_intercept(&self.fastabterm_state, cx);
    }
    ...
    fn sync_own_intercept_for_layout(&self, positioned: bool, cx: &App) {
        ...
        set_intercept_flags(&self.fastabterm_state, session_id, visible_intercept, global_intercept);
    }

    fn sync_intercept(&self, fastabterm_state: &FigtermState, cx: &App) {
        let Some(session_id) = self.current_session() else {
            return;
        };
        let overlay = self.state.read(cx);
    ...
    fn complete_buffer_inner(
        ...
        if switched_session {
            ...
            *self.last_position.lock().unwrap_or_else(|err| err.into_inner()) = None;
            self.park_window(cx);
            set_intercept(&fastabterm_state, session_id, false, false);
        }
        ...
        if suppress_insert_update {
            self.park_window(cx);
            self.sync_intercept(&fastabterm_state, cx);
            return;
        }
        ...
        let (current_shell, current_process, environment_variables, alias) = fastabterm_state
    ...
    pub fn handle_action(
        ...
        match action {
            "navigateUp" => self.move_selection(-1, true, fastabterm_state, cx),
            "navigateDown" => self.move_selection(1, false, fastabterm_state, cx),
            "hideAutocomplete" => {
                self.hide_until_shown(cx);
                self.sync_intercept(fastabterm_state, cx);
            },
            ...
            "insertSelected" => self.insert_selected(false, fastabterm_state, cx),
            "insertCommonPrefixOrInsertSelected" => {
                if !self.insert_common_prefix(fastabterm_state, cx) {
                    self.insert_selected(false, fastabterm_state, cx);
                }
            },
            "insertSelectedAndExecute" => self.insert_selected(true, fastabterm_state, cx),
            "execute" => {
                let _ = self.insert_text("\n", 0, true, fastabterm_state, cx);
            },
            "insertCommonPrefix" => {
                if !self.insert_common_prefix(fastabterm_state, cx) {
                    self.shake(cx);
            ...
            "insertCommonPrefixOrNavigateDown" => {
                if !self.insert_common_prefix(fastabterm_state, cx) {
                    self.move_selection(1, false, fastabterm_state, cx);
                }
            },
            "toggleDescription" => {
                if !fastab_settings::settings::get_bool_or("autocomplete.alwaysShowDescription", false) {
                    self.state.update(cx, |overlay, cx| {
    ...
    pub fn handle_click(
        ...
        self.insert_item(click, false, fastabterm_state, cx);
    }
    ...
    fn move_selection(&mut self, delta: i32, up_from_top: bool, fastabterm_state: &FigtermState, cx: &mut App) {
    ...
    fn insert_common_prefix(&mut self, fastabterm_state: &FigtermState, cx: &mut App) -> bool {
        let decision = {
            let overlay = self.state.read(cx);
        ...
        match decision {
            ...
            Some(TabPrefix::Partial(shared)) => {
                ...
                self.insert_text(&insertion, deletion, false, fastabterm_state, cx);
                true
            },
            None => false,
        }
    ...
    fn insert_selected(&mut self, execute: bool, fastabterm_state: &FigtermState, cx: &mut App) {
        let selected = {
            let overlay = self.state.read(cx);
            let Some(item) = overlay.selected_item() else {
                return;
        ...
        self.insert_item(selected, execute, fastabterm_state, cx);
    }

    fn insert_item(&mut self, item: ClickInsert, execute: bool, fastabterm_state: &FigtermState, cx: &mut App) {
        ...
        let inserted = self.insert_text(&insertion, deletion, execute, fastabterm_state, cx);
        if inserted
    ...
    fn insert_text(
        ...
        let Some(sender) = fastabterm_state.with(&session_id, |session| session.sender.clone()) else {
            warn!(%session_id, "no fastabterm session for insert");
            return false;
        };
...
fn apply_settings(overlay: &mut OverlayState) {
    ...
    let configured_width = fastab_settings::settings::get_int("autocomplete.width").ok().flatten();
    let configured_history_mode = fastab_settings::settings::get_string("beta.history.mode")
        .ok()
        .flatten();
    ...
    overlay.max_list_height =
        fastab_settings::settings::get_int_or("autocomplete.height", DEFAULT_MAX_LIST_HEIGHT as i64).max(40) as f32;
    overlay.fuzzy_search = fastab_settings::settings::get_bool_or("autocomplete.fuzzySearch", true);
    overlay.effective_fuzzy_search = overlay.fuzzy_search;
    ...
    overlay.always_show_description =
        fastab_settings::settings::get_bool_or("autocomplete.alwaysShowDescription", false);
...
fn overlay_metrics() -> OverlayMetrics {
    let configured_font = fastab_settings::settings::get_string("autocomplete.fontFamily")
        .ok()
        .flatten()
    ...
    let font_size = {
        let setting = fastab_settings::settings::get_int_or("autocomplete.fontSize", 0);
...
pub fn resolve_overlay_theme() -> OverlayTheme {
    let name = fastab_settings::settings::get_string_or("autocomplete.theme", "github-dark".into());
...
pub(crate) fn overlay_theme_by_name(name: &str) -> OverlayTheme {
    match name.to_ascii_lowercase().as_str() {
        ...
        "system" => {
            if fastab_gpui::system_appearance_is_dark() {
                OverlayTheme::dark()
            } else {
                OverlayTheme::light()
            }
...
fn load_named_theme(name: &str) -> Option<OverlayTheme> {
    ...
    let path = fastab_util::directories::themes_dir(&ctx)
        .ok()?
...
fn apply_complete_result(
    ...
    match result {
        Ok(result) => {
            ...
            set_intercept_flags(fastabterm_state, session_id, visible_intercept, global_intercept);
            if visible && positioned && has_items {
                fastab_telemetry::count("autocomplete_shown");
            }
            positioned
        },
        Err(err) => {
...
/// shell characters (the same units used by the backspaces sent to fastabterm).
...
fn set_intercept(fastabterm_state: &FigtermState, session_id: Uuid, overlay_visible: bool, has_items: bool) {
    let (intercept, intercept_global) = intercept_flags(overlay_visible, has_items);
    set_intercept_flags(fastabterm_state, session_id, intercept, intercept_global);
}

fn set_intercept_flags(fastabterm_state: &FigtermState, session_id: Uuid, intercept: bool, intercept_global: bool) {
    for session in fastabterm_state.inner.lock().linked_sessions.values_mut() {
...
fn place_overlay_at_caret(
    ...
    if let Some((left, top, right, bottom)) = edges {
        if popout && x + overlay_width > right {
            let shift = fastab_gpui::POPOUT_WIDTH as f64;
...
mod tests {
    ...
    fn common_prefix_of_nested_paths() {
        assert_eq!(
            fastab_gpui::longest_common_prefix(&["src/main.rs", "src/mod.rs"]),
            "src/m"
        );
    }
    ...
    fn popout_flips_to_the_left_when_the_combined_width_overflows() {
        let popout_shift = fastab_gpui::POPOUT_WIDTH as f64;
