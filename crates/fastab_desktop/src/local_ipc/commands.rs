...
pub async fn diagnostic(_: DiagnosticsCommand, fastabterm_state: &FigtermState) -> LocalResult {
    let (edit_buffer_string, edit_buffer_cursor, shell_context, intercept_enabled, intercept_global_enabled) = {
        match fastabterm_state.most_recent() {
            Some(session) => (
...
pub fn dump_state(
    ...
    let json = match command.r#type() {
        DumpStateType::DumpStateFigterm => {
            serde_json::to_string_pretty(&fastabterm_state).unwrap_or_else(|err| format!("unable to dump: {err}"))
        },
