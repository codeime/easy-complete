...
use fastab_util::url::USER_MANUAL;
    ...
    match &*menu_event.id().0 {
        ...
        "uninstall" => {
            tokio::runtime::Handle::current().spawn(async {
                fastab_install::uninstall(InstallComponents::all(), Context::new())
                    .await
                    .ok();
                #[allow(clippy::exit)]
                std::process::exit(0);
...
pub async fn build_tray(
    ...
    _fastabterm_state: &FigtermState,
...
fn menu(is_logged_in: bool) -> Vec<MenuElement> {
    ...
    let mut menu = if !is_logged_in {
        ...
        let onboarded_completed = fastab_settings::state::get_bool_or("desktop.completedOnboarding", false);
