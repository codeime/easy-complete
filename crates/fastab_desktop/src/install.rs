...
#[cfg(target_os = "macos")]
use fastab_util::directories::fastab_data_dir;
#[cfg(target_os = "macos")]
...
pub async fn migrate_data_dir() {
    ...
    if let (Ok(old), Ok(new)) = (fastab_util::directories::old_fastab_data_dir(), fastab_data_dir()) {
...
pub fn record_accessibility_grant() {
    ...
    if fastab_settings::state::get_bool_or(ACCESSIBILITY_GRANTED_KEY, false) {
        return;
    }

    if let Err(err) = fastab_settings::state::set_value(ACCESSIBILITY_GRANTED_KEY, true) {
...
fn reconcile_accessibility_permission() {
    ...
    let previously_granted = fastab_settings::state::get_bool_or(ACCESSIBILITY_GRANTED_KEY, false);
    ...
    if previously_granted {
        ...
        if let Err(err) = fastab_settings::state::set_value(ACCESSIBILITY_GRANTED_KEY, false) {
...
pub async fn run_install(
    ...
    #[cfg(target_os = "macos")]
    {
        initialize_fastab_dir(&fastab_os_shim::Env::new()).await.ok();

        if fastab_util::directories::home_dir()
            .map(|home| home.join("Library/Application Support/fig/credentials.json"))
            .is_ok_and(|path| path.exists())
            && !fastab_settings::state::get_bool_or(MIGRATED_KEY, false)
        {
            fastab_settings::state::set_value(MIGRATED_KEY, true).ok();
            // fastab_telemetry removed
        }
    }
    ...
    if should_run_macos_install {
        ...
        if previous_version().is_none() {
            fastab_telemetry::track("app_installed");
        } else {
            fastab_telemetry::track_with_props(
                "app_updated",
                serde_json::json!({
    ...
    fastab_telemetry::track_with_props("app_opened", serde_json::json!({ "is_startup": is_startup }));

    #[cfg(target_os = "linux")]
    run_linux_install(
        Arc::clone(&ctx),
        Arc::new(fastab_settings::Settings::new()),
        Arc::new(fastab_settings::State::new()),
    )
    .await;
...
#[cfg(target_os = "macos")]
pub async fn initialize_fastab_dir(env: &fastab_os_shim::Env) -> anyhow::Result<()> {
    use std::fs;
    ...
    use fastab_util::{OLD_CLI_BINARY_NAMES, OLD_PTY_BINARY_NAMES, Shell};
    use tracing::warn;
    ...
    // Install fastabterm to ~/.local/bin
    ...
    match get_bundle_path_for_executable(CLI_BINARY_NAME) {
        Some(fastab_cli_path) => {
            let dest = local_bin.join(CLI_BINARY_NAME);
            if let Err(err) = symlink(&fastab_cli_path, dest).await {
            ...
            for old_cli_binary_name in OLD_CLI_BINARY_NAMES {
                ...
                if old_cli_binary_path.is_symlink() {
                    if let Err(err) = symlink(&fastab_cli_path, &old_cli_binary_path).await {
    ...
    std::fs::create_dir(fastab_data_dir()?.join("shell")).ok();
    for shell in fastab_util::Shell::all().iter() {
...
#[cfg(target_os = "linux")]
async fn run_linux_install(
    ctx: Arc<Context>,
    settings: Arc<fastab_settings::Settings>,
    state: Arc<fastab_settings::State>,
) {
    use dbus::gnome_shell::ShellExtensions;
    use fastab_settings::State;
    use fastab_util::system_info::linux::get_display_server;
    ...
    match get_display_server(&ctx) {
        Ok(_) => (),
        Err(fastab_util::Error::UnknownDisplayServer(server)) => {
            error!(
...
where
    Ctx: fastab_os_shim::ContextProvider,
    ExtensionsCtx: fastab_os_shim::ContextProvider,
{
    use dbus::gnome_shell::{ExtensionInstallationStatus, get_extension_status};
    use fastab_os_shim::FsProvider;
    use fastab_util::directories::{bundled_gnome_extension_version_path, bundled_gnome_extension_zip_path};
    use fastab_util::system_info::linux::{DisplayServer, get_display_server};
    use tracing::debug;
...
#[cfg(target_os = "linux")]
async fn install_desktop_entry(ctx: &Context, state: &fastab_settings::State) -> anyhow::Result<()> {
    use fastab_integrations::desktop_entry::DesktopEntryIntegration;
    use fastab_util::directories::{appimage_desktop_entry_icon_path, appimage_desktop_entry_path};
...
async fn install_autostart_entry(
    ctx: &Context,
    settings: &fastab_settings::Settings,
    state: &fastab_settings::State,
) -> anyhow::Result<()> {
    use fastab_integrations::desktop_entry::{AutostartIntegration, should_install_autostart_entry};
...
async fn install_appimage_binaries(ctx: &Context) -> anyhow::Result<()> {
    use fastab_util::consts::{CLI_BINARY_NAME, PTY_BINARY_NAME};
    use fastab_util::directories::home_local_bin_ctx;
...
fn previous_version() -> Option<Version> {
    fastab_settings::state::get_string(PREVIOUS_VERSION_KEY)
        .ok()
        .flatten()
...
fn set_previous_version(version: Version) -> anyhow::Result<()> {
    fastab_settings::state::set_value(PREVIOUS_VERSION_KEY, version.to_string())?;
    Ok(())
}
...
mod test {
    ...
    mod linux_appimage_tests {
        ...
        use fastab_util::directories::home_local_bin_ctx;
        use fastab_util::{CLI_BINARY_NAME, PTY_BINARY_NAME};
        use tokio::process::Command;
...
echo "{binary_name} {version}"
    ...
    mod linux_desktop_entry_tests {
        use fastab_integrations::desktop_entry::{AutostartIntegration, local_entry_path, local_icon_path};
        use fastab_settings::{Settings, State};
        use fastab_util::directories::{appimage_desktop_entry_icon_path, appimage_desktop_entry_path};
