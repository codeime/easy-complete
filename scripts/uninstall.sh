...
if [[ "${1:-}" != "--yes" ]]; then
  ...
  echo "  • ${IME_SYMLINK}"
  echo "  • ${PLIST_PATH}"
  echo "  • ${LOCAL_BIN}/ftab  and  fastabterm"
  echo "  • ${APP_SUPPORT}/"
  echo "  • ${LOGS_DIR}/"
...
pkill -f "fastab_input_method"  2>/dev/null || true
...
rm -rf "${TMPDIR}fastabrun"  2>/dev/null || true
...
# ── Done ───────────────────────────────────────────────────────────────────────
echo ""
info "${APP_DISPLAY} has been fully uninstalled."
echo ""
