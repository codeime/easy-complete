...
use fastab_proto::local::{EditBufferHook, InterceptedKeyHook, PostExecHook, PreExecHook, PromptHook, ShellContext};
use fastab_proto::remote::clientbound;
use fastab_remote_ipc::RemoteHookHandler;
use fastab_remote_ipc::fastabterm::FigtermState;
use fastab_util::RUNTIME_DIR_NAME;
...
impl Shell {
    pub async fn init(shell: &str) -> Result<Shell> {
        ...
        tokio::spawn({
            ...
            async move {
                fastab_remote_ipc::remote::start_remote_ipc(
                    path,
