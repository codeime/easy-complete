...
- 分支：`fix/cross-platform-audit-1` @ `81d4ea7b`（从 `feat/cross-platform` 切出；`feat/cross-platform` 来自 `main` @ `55b043ff`）
...
- 下一步：push 后看第一次 `rust-linux` / `rust-windows`（现在是整套 `fig_desktop` + `fig_log`，不是 caret/overlay 过滤器）；Windows 手测 named pipe 往返、ConPTY、caret、HWND。不要为了翻 skip 去给 Ubuntu 装 shellcheck 或 dbus-daemon，不要装 WebKit，不要把 `mesa-vulkan-drivers` 写进 rust-linux job，**不要复活考古 `platform/linux/` / `platform/windows.rs`**，不要把 `webview/` 目录名请回来。N1 / N2 / N3 / N4 / N5 已关。`match_buckets_follow_webview_exact_prefix_order` 已关。macos `focused_window.lock().unwrap()` 与 `tray.rs` `get_icon` expect 已关。fig_ipc A3 帧 resync **研究关闭，不改 framing**。macos `event_sender` / `window_server` RwLock poison、`file_watcher` `recommended_watcher` unwrap、overlay `cached_image` expect 已关。省约 R3（figterm intercept unwrap、ecterm blocking=8、`AutocompleteFlags` 一次 load、Linux overlay cache/intern/dpi）已关。figterm 本地 IPC send unwrap、fig_log poison、JS `executeCommand` Unix poll、macos SetEnabled / event_sender Option unwrap、用户可见 qterm copy 已关。ranking 预计算 + ASCII `match_score` + filegen sort/stat + IME/AX 政策进 caret.rs + rust-linux/windows 整套 fig_desktop/`fig_log` 已关。留下 macos `GLOBAL_PROXY.get().unwrap()`（OnceLock）、`CString::new`（NUL）。Windows `execute_full` 仍 `wait_with_output` 辅助线程（无 poll）。未 push。
